...
<script type="text/javascript" charset="utf-8">
	...
	function append() {
		var p = parent.sy.dialog({
			title : 'Add document',
			href : '${pageContext.request.contextPath}/docAction!docAdd.action',
			width : 500,
			height : 200,
			buttons : [ {
				text : 'Add',
				handler : function() {
					var f = p.find('form');
					f.form('submit', {
						url : '${pageContext.request.contextPath}/docAction!add.action',
						success : function(d) {
	...
	function edit() {
		...
		if (rows.length == 1) {
			var p = parent.sy.dialog({
				title : 'Edit document',
				href : '${pageContext.request.contextPath}/docAction!docEdit.action',
				width : 500,
				height : 200,
				buttons : [ {
					text : 'Edit',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/docAction!edit.action',
							success : function(d) {
								var json = $.parseJSON(d);
								if (json.success) {
									datagrid.datagrid('reload');
		...
		if (rows.length > 0) {
			parent.sy.messagerConfirm('Confirm', 'Are you sure to delete', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/docAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
	...
	function Upload() {
		...
		if (rows.length == 1) {
			var p = parent.sy.dialog({
				title : 'Upload manual',
				href : '${pageContext.request.contextPath}/docAction!docUpload.action',
				width : 400,
				height : 160,
				buttons : [ {
					text : 'Upload',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/docAction!upload.action',
							success : function(d) {
								var json = $.parseJSON(d);
								if (json.success) {
									datagrid.datagrid('reload');

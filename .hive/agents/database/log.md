...
## [2026-03-13 11:05:28] t24: Fix production role name mismatch: V3 migration to rename 'Administrator' to 'ADMIN' in t_role seed data
`UserDetailsServiceImpl` uppercases role names before prepending `ROLE_`. All seed data role names must be uppercase to match `@PreAuthorize("hasRole('...')")` annotations. Tests use `TestDataSeeder` which had correct names — always check that test fixtures match migration seeds.
